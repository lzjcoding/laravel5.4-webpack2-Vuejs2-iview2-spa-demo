...
<template>
    <div class="layout" :class="{ 'layout-hide-text': isHideText }">
        <div type="flex">
            <div class="layout-menu-left" :class="isHideText ? 'only-icon' : 'has-text'" :style="menuLeftStyle">
                <Menu :active-key="activeKey" theme="dark" width="auto">
                    <div class="layout-logo-left"></div>
                    <Menu-item key="post-index" v-link="{ name: 'post-index' }">
                        <Icon type="ios-paper" :size="iconSize"></Icon>
                        <span class="layout-text">文章管理</span>
                    </Menu-item>
                    <Menu-item key="user-index" v-link="{ name: 'user-index' }">
        ...
        <div class="layout-copy">
            2016-{{ new Date().getFullYear() }} &copy; Lzj
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            fetchUserData () {
                this.$http.get(config.api.user_info).then(response => {
                    this.authUser.name = response.body.data.name;
                });
            },
            logout () {
                this.$http.post(config.api.logout).then(response => {
